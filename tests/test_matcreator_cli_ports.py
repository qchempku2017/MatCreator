...
def test_cli_config_set_mcp_ports(monkeypatch, tmp_path: Path) -> None:
    """Set each MCP port via CLI and verify the values are stored.

    The existing config system uses dotted-key notation (section.field).
    Keys with more than two segments (e.g. ports.mcp.database) are flattened:
    ``ports.setdefault("mcp", {})["database"]`` is NOT created; instead
    ``ports["mcp.database"]`` is stored as a flat key because
    ``dotted_key.split(".", 1)`` only splits once.

    This test uses the ``ports.mcp_<name>`` convention as a 2-level key,
    which is compatible with the current config set/get implementation.
    """
    _redirect_config_home(monkeypatch, tmp_path)
    runner = CliRunner()

    mcp_ports = {
        "ports.mcp_database": "51001",
        "ports.mcp_dpa": "51002",
        "ports.mcp_abacus": "51003",
        "ports.mcp_structure": "51004",
        "ports.mcp_vasp": "51005",
        "ports.mcp_mattergen": "51006",
    }

    for key, value in mcp_ports.items():
        result = runner.invoke(start_agent.main, ["config", "set", f"{key}={value}"])
        assert result.exit_code == 0, f"Failed to set {key}: {result.output}"

    # Verify via config get for each key
    for key, expected_value in mcp_ports.items():
        result = runner.invoke(start_agent.main, ["config", "get", key])
        assert result.exit_code == 0, f"Failed to get {key}"
        assert expected_value in result.output, f"Key {key}: expected {expected_value}, got: {result.output}"


def test_cli_config_set_mcp_nested_ports(monkeypatch, tmp_path: Path) -> None:
    """Set a 3-level nested port key (ports.mcp.database) via CLI and verify.

    After the config.py set_config_value fix, dotted keys with more than 2
    segments are written as truly nested YAML dicts::

        ports:
          mcp:
            database: "51001"

    instead of the previous flat ``ports["mcp.database"]`` convention.
    """
    _redirect_config_home(monkeypatch, tmp_path)
    runner = CliRunner()

    # Set a 3-level dotted key
    result = runner.invoke(
        start_agent.main, ["config", "set", "ports.mcp.database=51001"]
    )
    assert result.exit_code == 0, f"Failed to set ports.mcp.database: {result.output}"

    # Verify via config get (back through get_config_value)
    result = runner.invoke(start_agent.main, ["config", "get", "ports.mcp.database"])
    assert result.exit_code == 0
    assert "51001" in result.output, f"Expected 51001, got: {result.output}"

    # Also verify the raw YAML structure is properly nested (not a flat key)
    raw = _read_raw_yaml()
    ports = raw.get("ports", {})
    mcp = ports.get("mcp", {})
    assert mcp.get("database") == "51001", (
        f"Expected ports.mcp.database=51001 (nested dict), "
        f"but got raw YAML: {raw}"
    )

    # And confirm no flat "mcp.database" key exists inside ports
    assert "mcp.database" not in ports, (
        f"Flat key 'mcp.database' should not exist inside ports; "
        f"ports should contain nested 'mcp' dict. Got: {list(ports.keys())}"
    )
